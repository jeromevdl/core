...
class object
{
    ...
    public static function all($_onlyVisible = false)
    {
        ...
        $sql .= ' ORDER BY father_id,position,id';
    ...
    static function createTree(&$list, $parents)
    {
        $tree = array();
        foreach ($parents as $parent) {
            $obj = utils::o2a($parent);
            if (isset($list[$obj['id']])) {
                $obj['children'] = object::createTree($list, $list[$obj['id']]);
            }
            $tree[] = $obj;
        }
        return $tree;
    ...
    public static function tree($_visible = true) {
        $flat = object::all($_visible);
        $grouped = array();
        foreach ($flat as $object) {
            if (is_null($object->father_id)) {
                $grouped[0][] = $object;
            } else {
                $grouped[$object->father_id][] = $object;
            }
        }
        $tree = object::createTree($grouped, $grouped[0]);
        return $tree;
    }
